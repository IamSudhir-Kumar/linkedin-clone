...
const RegisterPageComponent = () => {
  ...
  const register = async () => {
    try {
      ...
      getUserName({name: credentails.name, email: credentails.email})
          ...
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6" action="#">
              <div>
                ...
                <input type="text" name="text" id="text" className="bg-gray-50 border 
                border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 
                focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600
                 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" 
                 placeholder="type your name" required="" onChange={(event) => setCredentails({ ...credentails, name: event.target.value })} />
              </div>
              <div>
                <label htmlFor="email" 
                ...
                placeholder="name@company.com" required="" onChange={(event) => setCredentails({ ...credentails, email: event.target.value })} />
              </div>
              <div>
